...
// Defines the characters to be used in the passwords  
...
function getPrompts() {
  ...
  arrayChar = []; // Clears the prompt character option selection on refresh
  ...
  if (confirm("Lowercase? Yes or No?")) {
    arrayChar = arrayChar.concat(lowerCase);
  }
  // Uppercase prompt  
  if (confirm("Uppercase? Yes or No")) {
    arrayChar = arrayChar.concat(upperCase);
  }
  // Numbers prompt    
  if (confirm("Numbers? Yes or No?")) {
    arrayChar = arrayChar.concat(Numbers);
  }
  // Special Characters prompt
  if (confirm("Special Characters? Yes or No?")) {
    arrayChar = arrayChar.concat(specialChar)
  }
  return true;
}
...
function generatePassword() {
  ...
  for (var i = 0; i < passwordLength; i++) {
    var randomLetter = Math.floor(Math.random() * arrayChar.length);
    password = password + arrayChar[randomLetter];
  }
  return password;
}